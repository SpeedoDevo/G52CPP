...
void BaseEngine::CreateObjectArray(int iNumberObjects)
{
	...
	DestroyOldObjects();
	DisplayableObjects = std::deque<DisplayableObject*>(iNumberObjects + 1);
...
void BaseEngine::StoreObjectInArray(int iIndex, DisplayableObject* pObject)
{
	...
	{
		if (iIndex == -1) {
			DisplayableObjects.push_back(pObject);
		}
		else
		{
			DisplayableObjects.at(iIndex) = pObject;
		}
	}
}
...
*/
	...
	// avoid undrawing if background doesn't exist
	if (m_iScreenWidth < iX || m_iScreenHeight < iY) return;
	iX = iX < 0 ? 0 : iX;
	iY = iY < 0 ? 0 : iY;
	//
...
DisplayableObject* BaseEngine::GetDisplayableObject( int iIndex )
{
	if ( &DisplayableObjects != NULL )
	{
		return DisplayableObjects[iIndex];
	}
	else
		return NULL;
}
									...
									unsigned int uiColour, SDL_Surface* pTarget)
{
	...
	//////////////////////////////////////////////////////////////////////////
	//changed
	//don't calculate for offscreen values
	if (iX1 < 0) iX1 = 0;
	if (iY1 < 0) iY1 = 0;
	if (m_iScreenWidth < iX2) iX2 = m_iScreenWidth;
	if (m_iScreenHeight < iY2) iY2 = m_iScreenHeight;
	//////////////////////////////////////////////////////////////////////////
...
int BaseEngine::NotifyAllObjectsGetSum( int iSignalNumber )
{
	...
	if ( &DisplayableObjects != NULL )
	{
		...
		{
			if (ob == NULL) break;
			iReturn += ob->Notify( iSignalNumber );
		}
	}
	return iReturn;
}
...
int BaseEngine::NotifyAllObjectsGetMax( int iSignalNumber )
{
	...
	if ( &DisplayableObjects != NULL )
	{
		...
		{
			if (ob == NULL) break;
			int ival = ob->Notify( iSignalNumber );
			if ( ival > iReturn )
				iReturn = ival;
		}
	}
